...
def get_transport_simulation_by_passive(passive_nuclear_molar_rate_per_sec,
    ...
    ts.set_params(**kwargs) # override defaults
    self.set_RAN_distribution(Ran_cell_M= Ran_cell_M, # total physiological concentration of Ran # TODO: check in the literature 
                                  parts_GTP_N=1000,
                                  parts_GTP_C=1,
                                  parts_GDP_N=1,
                                  parts_GDP_C=1000)
...
def my_plot_param_grid(df, # a pivoted 2D dataframe
    ...
    ax = plt.gca()
    c = plt.contourf( X, Y, df.to_numpy(), 
                **contourf_kwargs)
    ax.set_xscale('log')
    ax.set_yscale('log')
...
```{python}
import map_param_gridaram_grid
