...
def get_ts_time_series_3(passive_nuclear_molar_rate_per_sec,
    ...
    ts.rate_GDP_N_to_GTP_N_per_sec= 500.0
    ts.rate_GTP_N_to_GDP_N_per_sec= 0.1
    ts.rate_GTP_C_to_GDP_C_per_sec= 50.0
    ts.rate_GTP_N_to_GTP_C_per_sec = 0.5
    ts.rate_GDP_C_to_GDP_N_per_sec = 0.4
    ts.rate_GDP_N_to_GDP_C_per_sec = 0.4 
    ts.rate_complex_to_free_per_sec = 5e-2
    ts.rate_free_to_complex_per_sec = 0.3e+1
    ts.fraction_complex_NPC_traverse_per_sec=200
    ts.set_params(**kwargs) # override defaults
    return ts
```
...
def get_transport_simulation_by_passive(passive_nuclear_molar_rate_per_sec,
    ...
    ts.set_NPC_dock_sites(n_NPCs= 2000, 
                        n_dock_sites_per_NPC= 500)
    ts.set_passive_nuclear_molar_rate_per_sec(passive_nuclear_molar_rate_per_sec) #get_passive_export_rate_per_sec(27,1))
    ts.fraction_complex_NPC_to_free_N_per_M_GTP_per_sec = 2.5e+6 # TODO: this is doubled relative to complex_N to free_N
    ts.fraction_complex_N_to_free_N_per_M_GTP_per_sec = 2.5e+6
    ts.rate_complex_to_NPC_per_free_site_per_sec_per_M= 5e+6
    ts.fraction_complex_NPC_to_complex_N_C_per_sec= 30.0 # Leakage parameter
    ts.rate_GDP_N_to_GTP_N_per_sec= 100.0
    ts.rate_GTP_N_to_GDP_N_per_sec= 0.1
    ts.rate_GTP_C_to_GDP_C_per_sec= 100.0
    ts.rate_GTP_N_to_GTP_C_per_sec = 0.1
    ts.rate_GDP_C_to_GDP_N_per_sec = 30.0
    ts.rate_GDP_N_to_GDP_C_per_sec = 30.0 
    return ts
...
```{python}
param_range= get_param_range_traverse_kon(nx= 5, ny= 5)
print(param_range)
n_processors= multiprocessing.cpu_count()
if True:
...
```{python}
keys= sorted(stats_grids_traverse_by_passive_force.keys(), key=lambda x: (x[1],x[0]))
for key in keys:
    if key[1]:
        continue
