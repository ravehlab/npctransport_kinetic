...
def get_param_range_traverse_kon(nx, ny):
    ...
    param_range['range_x']= np.logspace(2, 7, nx) # divided by 50E-6 to normalize by the initial cargo concentration # TODO: take the actual cargo concentration?
    param_range['tag_y']= "rate_free_to_complex_per_sec"
    param_range['range_y']= np.logspace(-3, 0, ny)
    return param_range
...
param_range= get_param_range_D_kon(nx= 3, ny= 4)
stats_grids= map_param_grid.map_param_grid_parallel( param_range,
...
# Bound fraction
fig, axes= plt.subplots(2,1, figsize=(4.5,9), sharex=True, sharey=True)
map_param_grid.plot_bound_fraction(param_range, stats_grids, 'N', ax=axes[0])
map_param_grid.plot_bound_fraction(param_range, stats_grids, 'C', ax=axes[1])
# Import/export
plot_import_export()
