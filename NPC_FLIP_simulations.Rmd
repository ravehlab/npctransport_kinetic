...
def get_ts_parameters(MW= 27, 
                      ...
                      **kwargs):
    ts = TransportSimulation(**kwargs)
    ts.set_passive_diffusion_rate_per_sec(
        get_passive_diffusion_rate(MW, is_force))
...
def get_passive_diffusion_rate(MW, is_force):
...
def get_compartment_nmol_stats(ts, 
    ...
    for state in ['complex']: #['free','complex']:
        for label in labels:
            tag= '{}{}_{}'.format(state, 
                                  label, 
                                  compartment)
...
fig, ax_grid= plt.subplots(5, 2, figsize=(10,10), sharex=True, sharey='row')
ax_grid= ax_grid.transpose()
for axes, is_force in zip(ax_grid, [False, True]):
    for i_NLS in range(1):
        ts = get_ts_parameters(MW=41,
                                NLS_strength=i_NLS,
                              is_force= is_force)
        ts.set_params(dt_sec=1e-3)
        simulation_time_sec= 100
#        ts.bleach_start_time_sec= simulation_time_sec/2.0
        ...
        ys[2]= get_N_C_ratio_stats(ts,
                                   stats, 
                                   labels)
        ys[3]= get_compartment_nmol_stats(ts, 
                                                  stats, 
                                                  'C', 
                                                  labels)
        ys[4]= get_compartment_nmol_stats(ts, 
                                                  stats, 
                                                  'N', 
                                                  labels)
        plot_from_sec=  1.0 # ts.bleach_start_time_sec + 1.0
